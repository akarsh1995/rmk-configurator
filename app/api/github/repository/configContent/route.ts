...
import { commitMultipleFilesToRepo, getKeyboardToml, getVialJson } from "../../../../../lib/github/repository";
  ...
  try {
    ...
    if (owner && repo && keyaboardToml && vialJson) {
      await commitMultipleFilesToRepo({
        ...
        repo,
        branch: 'main',
        files: [{ path: RmkFilePaths.KEYBOARD_TOML, content: stringify(keyaboardToml) }, { path: RmkFilePaths.VIAL_JSON, content: JSON.stringify(vialJson) }],
        message: 'updates',
        authToken: accessToken
      });
